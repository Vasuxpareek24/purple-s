...
PANELS = {
    "panel1": {
        "name": "Konekta Panel",
        "login_url": "http://15.235.182.3/konekta/signin",
        "xhr_url": "http://15.235.182.3/konekta/agent/res/data_smscdr.php?fdate1=2026-01-24%2000:00:00&fdate2=2027-01-24%2023:59:59&frange=&fclient=&fnum=&fcli=&fgdate=&fgmonth=&fgrange=&fgclient=&fgnumber=&fgcli=&fg=0&sEcho=1&iColumns=9&sColumns=%2C%2C%2C%2C%2C%2C%2C%2C&iDisplayStart=0&iDisplayLength=25&mDataProp_0=0&sSearch_0=&bRegex_0=false&bSearchable_0=true&bSortable_0=true&mDataProp_1=1&sSearch_1=&bRegex_1=false&bSearchable_1=true&bSortable_1=true&mDataProp_2=2&sSearch_2=&bRegex_2=false&bSearchable_2=true&bSortable_2=true&mDataProp_3=3&sSearch_3=&bRegex_3=false&bSearchable_3=true&bSortable_3=true&mDataProp_4=4&sSearch_4=&bRegex_4=false&bSearchable_4=true&bSortable_4=true&mDataProp_5=5&sSearch_5=&bRegex_5=false&bSearchable_5=true&bSortable_5=true&mDataProp_6=6&sSearch_6=&bRegex_6=false&bSearchable_6=true&bSortable_6=true&mDataProp_7=7&sSearch_7=&bRegex_7=false&bSearchable_7=true&bSortable_7=true&mDataProp_8=8&sSearch_8=&bRegex_8=false&bSearchable_8=true&bSortable_8=false&sSearch=&bRegex=false&iSortCol_0=0&sSortDir_0=desc&iSortingCols=1&_=1769237395671",
        "username": PANEL1_USERNAME,
        "password": PANEL1_PASSWORD,
        "login_page": "http://15.235.182.3/konekta/sign-in",
        "referer": "http://15.235.182.3/konekta/sign-in",
        "ajax_referer": "http://15.235.182.3/konekta/agent/SMSCDRStats",
        "success_check": "SMSCDRStats",
    ...
    "panel2": {
        ...
        "username": PANEL2_USERNAME,
        "password": PANEL2_PASSWORD,
        "login_page": "http://217.23.5.21/ints/login",
        "referer": "http://217.23.5.21/ints/login",
        "ajax_referer": "http://217.23.5.21/ints/agent/SMSDashboard",
        "success_check": "SMSCDRStats",
        "session": requests.Session()
    }
}
...
for panel_id, config in PANELS.items():
    config["session"].headers.update({"User-Agent": "Mozilla/5.0"})
...
def init_database():
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_panel ON otp_records(panel_source)')
...
def save_otp_to_db(record, hash_id, panel_source):
    try:
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT OR IGNORE INTO otp_records 
                (hash_id, number, sender, message, otp_code, country, timestamp, panel_source)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                hash_id,
                record.get("num", ""),
...
def otp_processor_worker():
    ...
    while True:
        try:
            ...
            panel_source = record.get("panel_source", "unknown")
            save_otp_to_db(record, hash_id, panel_source)
            ...
            if otp:
                keyboard.add(types.InlineKeyboardButton(f"🔐 {otp}", callback_data=f"copy_{otp}"))
            
            keyboard.add(types.InlineKeyboardButton("📨 View Full SMS", callback_data=f"fullsms_{hash_id}"))
            ...
            if OTP_GROUP_IDS:
                for group_id in OTP_GROUP_IDS:
                    try:
                        sent_msg = bot.send_message(group_id, msg_group, reply_markup=keyboard, parse_mode="HTML")
            ...
            for chat_id in assigned_users:
                ...
                if otp:
                    personal_kb.add(types.InlineKeyboardButton(f"🔐 {otp}", callback_data=f"copy_{otp}"))
...
def login_panel(panel_id):
    ...
    try:
        ...
        res = session.get(config["login_page"], headers=headers, timeout=15)
    except Exception as e:
        logger.error(f"[{config['name']}] Login page request failed: {e}")
        return False
    ...
    if not match:
        logger.error(f"[{config['name']}] ❌ Captcha not found.")
        return False

    a, b = int(match.group(1)), int(match.group(2))
    captcha_answer = str(a + b)
    logger.info(f"[{config['name']}] ✅ Captcha solved: {a} + {b} = {captcha_answer}")
    ...
    try:
        res = session.post(config["login_url"], data=payload, headers=headers, timeout=15)
    except Exception as e:
        logger.error(f"[{config['name']}] Login POST failed: {e}")
        return False

    if config["success_check"] not in res.text:
        logger.error(f"[{config['name']}] ❌ Login failed.")
        return False

    logger.info(f"[{config['name']}] ✅ Logged in successfully.")
    return True

def fetch_otps_from_panel(panel_id):
    """Fetch OTPs from a specific panel"""
    config = PANELS[panel_id]
    session = config["session"]
    
    logger.info(f"🚀 [{config['name']}] OTP Monitor Started...")
    
    if not login_panel(panel_id):
        logger.error(f"[{config['name']}] ❌ Initial login failed. Exiting OTP loop.")
        return
    ...
    while True:
        try:
            ajax_headers = {
                ...
                "Referer": config["ajax_referer"]
            }
            
            res = session.get(config["xhr_url"], headers=ajax_headers, timeout=15)
            try:
                data = res.json()
            except Exception as e:
                logger.debug(f"[{config['name']}] Invalid JSON from XHR: {e}")
                time.sleep(1.5)
                continue
            ...
            for row in otps:
                try:
                    ...
                    hash_id = hashlib.md5((panel_id + str(number) + str(time_) + str(message)).encode()).hexdigest()
                    ...
                    logger.info(f"[{config['name']}] 📱 New OTP: {number} | {sender} | {otp_code or 'N/A'}")

                except Exception as e:
                    logger.debug(f"[{config['name']}] Row parse error: {e}")

        except Exception as e:
            logger.error(f"[{config['name']}] ❌ Error fetching OTPs: {e}")
            try:
                if res is not None and getattr(res, 'status_code', None) == 401:
                    logger.info(f"[{config['name']}] Attempting to re-login...")
                    if not login_panel(panel_id):
                        logger.error(f"[{config['name']}] ❌ Re-login failed.")
            except Exception:
                pass
...
def handle_full_sms(call):
    ...
    try:
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT message, panel_source FROM otp_records WHERE hash_id = ?', (hash_id,))
            row = cursor.fetchone()
            
            if row:
                message = row[0] or 'No message'
                panel = row[1] or 'unknown'
                panel_name = PANELS.get(panel, {}).get('name', 'Unknown Panel')
                full_msg = f"📱 {panel_name}\n\n{message}"
                bot.answer_callback_query(call.id, full_msg, show_alert=True)
            else:
...
def show_stats(message):
    ...
    try:
        with get_db() as conn:
            ...
            panel_stats = ""
            for panel_id, config in PANELS.items():
                cursor.execute('SELECT COUNT(*) FROM otp_records WHERE panel_source = ?', (panel_id,))
                panel_count = cursor.fetchone()[0]
                panel_stats += f"   • {config['name']}: {panel_count}\n"
            ...
            stats_text = (
                f"📊 <b>Multi-Panel Bot Statistics</b>\n\n"
                f"📱 <b>OTPs:</b>\n"
                f"   • Total: {total_otps}\n"
                f"   • Today: {otps_today}\n\n"
                f"🔌 <b>Panel-wise OTPs:</b>\n{panel_stats}\n"
                f"👥 <b>Users:</b>\n"
...
@bot.message_handler(commands=["panelstatus"])
def panel_status(message):
    """Check status of all panels"""
    if message.from_user.id != ADMIN_ID:
        return bot.reply_to(message, "❌ You are not the admin.")
    
    status_text = "🔌 <b>Panel Status:</b>\n\n"
    ...
    for panel_id, config in PANELS.items():
        try:
            with get_db() as conn:
                ...
                cursor.execute('''
                    SELECT COUNT(*) FROM otp_records 
                    WHERE panel_source = ? AND DATE(created_at) = DATE('now')
                ''', (panel_id,))
                today_count = cursor.fetchone()[0]
                
                cursor.execute('''
                    SELECT timestamp FROM otp_records 
                    WHERE panel_source = ? 
                    ORDER BY created_at DESC LIMIT 1
                ''', (panel_id,))
                last_row = cursor.fetchone()
                last_otp = last_row[0] if last_row else "Never"
            
            status_text += f"<b>{config['name']}</b>\n"
            status_text += f"   • Today's OTPs: {today_count}\n"
            status_text += f"   • Last OTP: {last_otp}\n"
...
def add_chat(message):
    ...
    response += "\n✅ All future OTPs from both panels will be sent to this group only!"
...
def admin_help(message):
    if message.from_user.id != ADMIN_ID:
        return bot.reply_to(message, "❌ You are not the admin.")
    help_text = """
🔧 <b>Multi-Panel Admin Commands:</b>
...
• /panelstatus - Check all panels status
...
<b>Note:</b> This bot monitors multiple panels simultaneously!
"""
...
if __name__ == "__main__":
    logger.info("🚀 Starting Multi-Panel OTP Bot...")
    ...
    # Start panel fetchers (one thread per panel)
    for panel_id in PANELS.keys():
        threading.Thread(
            target=fetch_otps_from_panel, 
            args=(panel_id,),
            daemon=True, 
            name=f"Panel-{panel_id}"
        ).start()
        logger.info(f"✅ Started {PANELS[panel_id]['name']} fetcher")
    ...
    logger.info(f"📡 Monitoring {len(PANELS)} panels simultaneously")
